...
public class MMS_lib {
	...
	public MMS_lib(double lambda2, double mu2, double s2) {
	...
	public double getL(){
		double l;
		l = Math.pow(this.s, this.s)*Math.pow(this.rho, this.s+1)*getP0()/getFactorial(this.s)/Math.pow(1-this.rho, 2)+alpha;
		return l;
	
	}
	public double getWq(){
		double wq;
		wq = Math.pow(this.s, this.s)*Math.pow(this.rho, this.s+1)*getP0()/getFactorial(this.s)/Math.pow(1-this.rho, 2)/lambda;
		return wq;
	...
	public double getW(){
		double w;
		w = Math.pow(this.s, this.s)*Math.pow(this.rho, this.s+1)*getP0()/getFactorial(this.s)/Math.pow(1-this.rho, 2)/lambda+1/mu;
		return w;
